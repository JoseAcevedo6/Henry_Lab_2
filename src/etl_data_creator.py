...
def split_rows_df(df, column, split):
    ...
    df_aux = df_aux[['title', 'id_service', 'cast', 'release_year', 'duration', 'listed_in']]
...
df_amazon = read_csv("datasets/amazon_prime_titles.csv")
df_amazon['id_service'] = 1
df_disney = read_csv("datasets/disney_plus_titles.csv")
df_disney['id_service'] = 2
df_hulu = read_csv("datasets/hulu_titles.csv")
df_hulu['id_service'] = 3
df_netflix = read_json("datasets/netflix_titles.json")
...
df_service = DataFrame({'id_service': [1, 2, 3, 4], 'service': ['amazon', 'disney', 'hulu', 'netflix']})

df_title = df_concated[['title']]
...
df_movie = df_movie.merge(df_title, how='left', on='title')

df_serie = split_rows_df(df_concated, 'type', 'TV Show')
df_serie = df_serie.merge(df_title, how='left', on='title')
